...
func (f *influxPublisher) Publish(contentType string, content []byte, config map[string]ctypes.ConfigValue) error {
	...
	for i, m := range metrics {
		...
		// NOTE: uint64 is specifically not supported by influxdb client due to potential overflow when we decode again later to an int64
		//without convertion of uint64 to int64, uint64 type will be treated as string by influxdb client
		if reflect.TypeOf(m.Data()).Kind() == reflect.Uint64 {
			tmpData := int64(m.Data().(uint64))
			data = tmpData
		}
